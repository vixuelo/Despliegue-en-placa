...
)
{
  ...
  status=rtems_semaphore_obtain(MutexId,RTEMS_WAIT,3333);
  for ( ; ; ) {
    ...
    status=rtems_semaphore_obtain(MutexId,RTEMS_NO_WAIT,3333);
    status = task_delay_until( task_index * 5 * get_ticks_per_second() );
  }
}
